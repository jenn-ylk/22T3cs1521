...
points_loop_cond:
	...
	mul	$t4, $t0, 8			# multiply by 8 to account for both row and col
	lw	$t1, my_points($t4)	# int row = my_points[i].row; (t1)
	add	$t4, $t4, 4
	lw	$t2, my_points($t4)	# int col = my_points[i].col; (t2)
	...
	add	$t4, $t4, $t2
	mul	$t4, $t4, 4
	lw	$t3, topography_grid($t4)		# load word from 0($s1) into $t1	$t3, 4				# int height = topography_grid[row][col]; (t3)
